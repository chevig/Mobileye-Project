...
def get_light_positions(c_image):
    ...
    position = np.argwhere(filtered_img == convolved_img)
    position = list(filter(lambda l: convolved_img[l[0]][l[1]] > 0.1, position))
    x_pos = [p[0] for p in position]
    y_pos = [p[1] for p in position]
